...
OUT_DASH = DOCS / "paper_alert_watch_mode.json"
OUT_HEALTH = DOCS / "paper_alert_watch_mode_health.json"
OUT_STATE = STATE / "paper_alert_watch_mode.json"
...
def main() -> None:
    ...
    for row in rows:
        ...
        if eligible:
            new["watch_mode_status"] = "WATCH_ALERT_ELIGIBLE"
            new["watch_alert_eligible"] = True
            eligible_count += 1
        else:
            new["watch_mode_status"] = "WATCH_ONLY_BLOCKED"
            new["watch_alert_eligible"] = False
            new["watch_alert_block_reasons"] = reasons
            blocked_count += 1
    ...
    out_rows.sort(
        key=lambda r: (
            1 if r.get("watch_alert_eligible") else 0,
            f(r.get("score_v2")),
            price(r),
        ),
        reverse=True,
    ...
    if eligible_count == 0:
        warnings.append("no_watch_alert_eligible_rows_currently")
    ...
    health = {
        "schema_version": "paper_alert_watch_mode_health_v1",
        "generated_at": generated_at,
        "status": status,
        "blockers": blockers,
        "warnings": warnings,
        "rows": len(out_rows),
        "watch_alert_eligible": eligible_count,
        "watch_alert_blocked": blocked_count,
        ...
        "live_trading": False,
        "message": "Watch-only alerts may be shown for validated research setup. Orders remain disabled.",
    }

    payload = {
        "schema_version": "paper_alert_watch_mode_v1",
        "generated_at": generated_at,
        "status": status,
        "health": health,
        "rows": out_rows,
        "safety": {
            "watch_alerts_only": True,
            "trade_eligibility_enabled": False,
            "paper_order_allowed": False,
            "live_order_allowed": False,
            "order_submission": False,
