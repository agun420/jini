...
OUT_RESULTS = DOCS / "price_regime_focused_validation.json"
OUT_HEALTH = DOCS / "price_regime_focused_validation_health.json"
OUT_STATE = STATE / "price_regime_focused_validation.json"
...
def candidate_symbols(limit: int = 80) -> list[str]:
    ...
    for row in rows:
        ...
        if not sym or p is None or sv2 is None:
            continue

        if PRICE_MIN <= p <= PRICE_MAX:
            clean.append((sv2, sym, p))

    clean.sort(reverse=True)

    out = []
    for _, sym, _ in clean:
        if sym not in out:
            out.append(sym)
        if len(out) >= limit:
            break
...
def passes_setup(bars: list[Any], i: int) -> bool:
    ...
    if i < max(momentum_lookback + pullback_lookback + 2, 40):
        return False
...
def main() -> None:
    generated_at = now()
    days = int(os.getenv("PRICE_REGIME_VALIDATION_DAYS", "20"))
    symbol_limit = int(os.getenv("PRICE_REGIME_SYMBOL_LIMIT", "80"))
    ...
    if not symbols:
        blockers.append("no_price_regime_symbols")
    ...
    if len(passed) < 10:
        warnings.append("symbols_tested_below_10")
    if total_tests < 100:
        warnings.append("sample_below_100")
    if proxy_pf < 1.2:
        warnings.append("profit_factor_below_1_2")
    if avg_return <= 0:
        warnings.append("avg_return_not_positive")
    if wins <= losses:
        warnings.append("wins_not_above_losses")
    ...
    payload = {
        "schema_version": "price_regime_focused_validation_v1",
        ...
        "safety": {
            ...
            "live_trading": False,
            "purpose": "Price-regime focused validation only. Does not submit orders.",
        },
    }

    health = {
        "schema_version": "price_regime_focused_validation_health_v1",
        "generated_at": generated_at,
        "status": status,
        "blockers": blockers,
        "warnings": warnings,
